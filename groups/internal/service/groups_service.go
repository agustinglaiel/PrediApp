...
type GroupServiceInterface interface {
	...
	GetJoinRequests(ctx context.Context, groupID int) (dto.GroupJoinRequests, e.ApiError)
}
...
func (s *groupService) GetJoinRequests(ctx context.Context, groupID int) (dto.GroupJoinRequests, e.ApiError) {
	users, err := s.groupRepo.GetJoinRequests(ctx, groupID)
	if err != nil {
		return dto.GroupJoinRequests{}, e.NewInternalServerApiError("Error fetching join requests", err)
	}

	return dto.GroupJoinRequests{
		GroupId: groupID,
		Users:   users,
	}, nil
}
