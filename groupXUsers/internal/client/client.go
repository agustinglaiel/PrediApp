...
// Funcion para obtener si existe un grupo por su groupId del microservicio groups
func (c *HttpClient) GetGroupByID(groupId int) (int, error) {
	endpoint := fmt.Sprintf("http://localhost:8053/groups/%d", groupId)
...
func (c *HttpClient) GetGroupByGroupName(groupName string) (string, error) {
	endpoint := fmt.Sprintf("http://localhost:8053/groups/groupName/%s", groupName)
	resp, err := c.Get(endpoint)
	if err != nil {
		return "", err
	}
