...
import (
	model "admin/internal/model/users"
	e "admin/pkg/utils"
	"context"
	"log"
...
type UserRepository interface{
	...
	GetUsers(ctx context.Context, limit, offset int) ([]*model.User, e.ApiError)
	...
	UpdateUserRole(ctx context.Context, id uint, newRole string) e.ApiError
	DeactivateUserByID(ctx context.Context, id uint) e.ApiError
	ReactivateUserByID(ctx context.Context, id uint) e.ApiError
}
...
func (r *userRepository) GetUsers(ctx context.Context, limit, offset int) ([]*model.User, e.ApiError) {
...
// UpdateUserRole actualiza el rol de un usuario por su ID
func (r *userRepository) UpdateUserRole(ctx context.Context, id uint, newRole string) e.ApiError {
    if err := r.db.WithContext(ctx).Model(&model.User{}).Where("id = ?", id).Update("role", newRole).Error; err != nil {
        return e.NewInternalServerApiError("error updating user role", err)
    }
    return nil
}

// DeactivateUserByID desactiva un usuario por su ID sin eliminarlo
func (r *userRepository) DeactivateUserByID(ctx context.Context, id uint) e.ApiError {
    if err := r.db.WithContext(ctx).Model(&model.User{}).Where("id = ?", id).Update("is_active", false).Error; err != nil {
        return e.NewInternalServerApiError("error deactivating user by ID", err)
    }
    return nil
}

// ReactivateUserByID reactiva un usuario por su ID
func (r *userRepository) ReactivateUserByID(ctx context.Context, id uint) e.ApiError {
    if err := r.db.WithContext(ctx).Model(&model.User{}).Where("id = ?", id).Update("is_active", true).Error; err != nil {
        return e.NewInternalServerApiError("error reactivating user by ID", err)
    }
    return nil
}
