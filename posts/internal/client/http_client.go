...
func (c *HttpClient) GetUserByID(userID int) (bool, error) {
    endpoint := fmt.Sprintf("/users/%d", userID) // Ajustado para no repetir el puerto
    ...
    // Añadir token JWT (puedes obtenerlo del contexto o una variable)
    req.Header.Set("Authorization", "Bearer <tu-token>")
