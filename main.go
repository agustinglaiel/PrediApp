...
func main() {
	...
	// Lista de servicios con sus respectivas variables de entorno para el puerto
	...
	for dir, serviceURL := range services {
		go func(d string, serviceURL string) {
			// Extraer el puerto de la URL del microservicio
			parts := strings.Split(serviceURL, ":")
			if len(parts) < 3 {
				log.Printf("Invalid service URL for %s: %s", d, serviceURL)
				return
			}
			port := parts[2]
